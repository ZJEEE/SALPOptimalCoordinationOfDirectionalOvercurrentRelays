...
Directional Overcurrent Relays: CASE II"""
...
General_Data = pd.read_excel(f'{path_}/six_bus_system.xlsx', sheet_name='General_Data')

Specific_Data = pd.read_excel(f'{path_}/six_bus_system.xlsx', sheet_name='Specific_Data')
...
for ips in range(int(PS_min*power), int((PS_max + step)*power), int(step*power)):
    ps_available.append(ips/power)
...
ax.set_ylim([0.95 * best_solution['fs'][-1], 1.05 * best_solution['fs'][0]])
title = f'IEEE 6 BUS SYSTEM RESULTS'
...
graphic = plt.gcf()
graphic.savefig(f'{path_}/fig_6bus.png', format='png')
plt.show()
